...
import {
  ...
  type OnboardingRole as ApiOnboardingRole,
...
const ROLE_CONFIGS: Record<OnboardingRole, RoleConfig> = {
  organisateur: {
    ...
    subtitle:
      "Structurez votre espace pour passer de l'intention à l'événement publié.",
    icon: Sparkles,
    ...
    steps: [
      {
        ...
        description:
          "Indiquez les formats qui reviennent le plus souvent dans votre calendrier.",
        fields: [
          {
            type: "choices",
            key: "eventTypes",
      ...
      },
      {
        ...
        description:
          "Ces informations aideront les partenaires à vous reconnaître rapidement.",
        fields: [
          { type: "file", key: "avatar", label: "Photo de profil optionnelle" },
          {
            type: "text",
            key: "displayName",
            label: "Nom d'affichage / entreprise",
      ...
      },
      {
        ...
        description:
          "Un tableau de bord organisateur vous attend avec vos événements, invités et partenaires.",
        fields: [],
      },
    ],
  },
  prestataire: {
    ...
    subtitle:
      "Préparez une fiche claire pour recevoir des demandes alignées avec votre expertise.",
    icon: Camera,
    ...
    steps: [
      {
        ...
        description:
          "Choisissez la catégorie qui décrit le mieux votre service.",
        fields: [
          {
            type: "choices",
            key: "category",
      ...
      },
      {
        ...
        description:
          "Ajoutez une description courte, votre zone de service et un repère tarifaire.",
        fields: [
          { type: "file", key: "logo", label: "Logo ou photo" },
          {
            ...
            maxLength: 200,
            placeholder: "Une phrase nette sur votre signature.",
          },
          {
            type: "choices",
            key: "serviceArea",
          ...
          },
          {
            type: "text",
            key: "rate",
            label: "Tarif indicatif optionnel",
            placeholder: "À partir de 850 $",
          },
        ],
      },
      {
        ...
        description:
          "Vous pourrez enrichir vos photos et disponibilités depuis votre espace prestataire.",
        fields: [],
      },
    ],
  },
  gestionnaire: {
    ...
    subtitle:
      "Présentez votre espace pour transformer les recherches en réservations qualifiées.",
    icon: Building2,
    ...
    steps: [
      {
        ...
        description:
          "Décrivez le type d'espace et la capacité que vous pouvez accueillir.",
        fields: [
          {
            type: "text",
            key: "venueName",
            label: "Nom de l'espace",
            placeholder: "Maison Saint-Laurent",
          },
          {
            type: "choices",
            key: "venueType",
      ...
      },
      {
        ...
        description:
          "Donnez aux organisateurs les repères nécessaires pour se projeter.",
        fields: [
          {
            type: "text",
            key: "address",
            label: "Adresse / ville / quartier",
            placeholder: "Vieux-Montréal",
          },
      ...
      },
      {
        ...
        description:
          "Votre tableau de bord centralise fiche, calendrier et demandes de réservation.",
        fields: [],
      },
    ],
  },
...
export function OnboardingFlow({ role }: OnboardingFlowProps) {
  ...
  const handleComplete = async () => {
    ...
    try {
      const session = await authService.saveOnboarding(
        toApiRole(role),
      ...
      setSaveError(
        getUserFacingError(error, {
          fallback:
            "Impossible de sauvegarder votre onboarding. Vérifiez les informations saisies, puis réessayez.",
        }),
      );
    } finally {
      setIsSaving(false);
  ...
  return (
    <main className="min-h-screen bg-background mesh-gradient px-4 py-6 sm:px-8">
      <header className="mx-auto flex w-full max-w-5xl items-center justify-between py-2">
        ...
        <Link href="/tableau-de-bord" className="premium-button-ghost">
          Compléter plus tard
        </Link>
      </header>
        ...
        <aside className="space-y-6">
          ...
          <div>
            <p className="section-eyebrow">
              Étape {stepIndex + 1} sur {config.steps.length}
            </p>
            <h1 className="premium-heading mt-4 text-[clamp(34px,5vw,58px)]">
              {config.title}
            </h1>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {saveError && (
                <FormErrorAlert
                  error={saveError}
                  className="bg-white/80 shadow-card"
                />
              )}
...
function buildOnboardingPayload(state: FormState): OnboardingPayload {
  return Object.entries(state).reduce<OnboardingPayload>(
    (payload, [key, value]) => {
...
function normalizePayloadValue(
  key: string,
  value: string | string[],
): OnboardingPayloadValue | undefined {
      ...
      <div className="space-y-2">
        ...
        {field.maxLength && (
          <p className="text-xs text-on-surface-variant">
            {stringValue.length}/{field.maxLength} caractères
          </p>
        )}
      </div>
    );
    ...
    return (
      <div className="space-y-2">
        ...
        <input
          ...
          onChange={(event) =>
            onTextChange(field.key, event.target.files?.[0]?.name ?? "")
          }
        />
      </div>
    );
  ...
  return (
    <div className="premium-card space-y-5 p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {(summary.length > 0
          ? summary
          : [["status", "Profil prêt à compléter"]]
        )
          .slice(0, 4)
          .map(([key, value]) => (
            <div
