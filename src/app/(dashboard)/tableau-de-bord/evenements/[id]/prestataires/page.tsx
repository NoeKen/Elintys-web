...
export default function EventPrestatairesPage() {
  ...
  const addMutation = useMutation({
    mutationFn: () =>
      vendorRequestsService.create(id, {
        source: "manual",
    ...
    onError: (error) =>
      setFormError(
        getUserFacingError(error, {
          fallback:
            "Impossible d’ajouter ce prestataire. Vérifiez son nom et son courriel.",
        }),
      ),
  });
  ...
  const handleAddManual = () => {
    if (!manualName.trim() || !manualEmail.trim()) {
      setFormError({
        message: "Le nom et le courriel du prestataire sont obligatoires.",
        details: [],
      });
      return;
    }
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      {showForm && (
        <div className="rounded-xl p-4 mb-6 bg-surface space-y-3 shadow-card">
          ...
          {formError && <FormErrorAlert error={formError} />}
      ...
      {!isLoading && requests.length === 0 && (
        <p className="text-muted">
          Aucun prestataire ajouté pour cet événement.
        </p>
      )}
      ...
      <div className="space-y-3">
        {requests.map((req: VendorRequest) => {
          const vendorName =
            typeof req.vendor === "object" && req.vendor
              ...
              <div className="flex items-center gap-3">
                <span
                  className={cn(
                    ...
                    STATUS_COLORS[req.status] ?? "bg-surface text-muted",
                  )}
                >
